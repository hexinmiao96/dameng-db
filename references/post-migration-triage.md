...
| 应用 I/O | 中文文件名 ZIP、Excel 公式单元格、公式结果缓存、空行过滤、上传解析行列偏移 | 中低相关，属于迁移回归场景，纳入验收用例 |
...
迁移后问题台账里常混入文件导出和 Excel 解析问题。它们通常不是达梦 SQL 问题，但应纳入上线回归：

- ZIP 中文文件名：使用 UTF-8 编码和 language encoding flag；HTTP 下载使用 `filename*` 兼容中文文件名。
- Excel 公式单元格：EasyExcel 读取公式单元格可能拿不到显示值；关键模板导入/导出用 Apache POI `FormulaEvaluator` 或读取缓存显示值。
- Excel 公式结果缓存：要确认读取的是公式表达式、缓存显示值，还是重新计算结果；同一模板不要混用两种口径。
- 空行过滤保留：不能简单按“所有单元格为空”过滤，需要区分纯空行、公式结果为空、合并单元格占位行、模板说明行和业务保留空行。
- 上传解析：模板行号、列号、合并单元格和公式单元格都要有样例文件回归。
